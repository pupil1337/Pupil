...
namespace Pupil {
	...
	void ParticleSystem::OnUpdate(const Pupil::TimeStep& ts) {
	...
	void ParticleSystem::OnRender(const Pupil::OrthographicCamera& camera) {
		Pupil::Renderer2D::BeginScene(camera);
		for (auto& particle : m_ParticlePool) {
			if (!particle.Active) continue;
	...
	void ParticleSystem::Emit(const ParticleProps& particleProps) {
		...
		particle.Velocity.y += particleProps.VelocityVariation.x * (Random::Float() - 0.5f);
